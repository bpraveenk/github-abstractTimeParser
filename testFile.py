        ...
        try:
            with open(self.__filename,'rb') as csvtrainfile:
                ...
                for row in datareader:
                    ...
                    endtime = row['endTime']
                    trainTuple = (sentence,reftime,begintime,endtime)
            ...
            logger.error("Can not open file:{}, {}".format(self._filename,e))
...
class labeledSentencesProcessor():
    ...
    def __init__(self,sentences,numTerminalTags=1,numValidTags=2):
        self._baseLabels = [
                            "<ETPYS>", "<ETPYN>", "<ETPDS>","<ETPDN>", "<ETPMS>", "<ETPMN>", "<ETPHS>", "<ETPHN>", "<ETPmN>","<ETPmS>", "<ETPAP>","<ETPZ>",
                            "<BTPYS>", "<BTPYN>", "<BTPDS>","<BTPDN>", "<BTPMS>", "<BTPMN>", "<BTPHS>", "<BTPHN>", "<BTPmN>", "<BTPmS>", "<BTPAP>", "<BTPZ>",
                            "<TDPS>" , "<TDPN>" , "<TDSS>" , "<TDSN>" , "<TPRD>"
                            ]
...
def test():
    ...
    #Results Output
    outputfile = "results/outputTestSentences_"+str(windowSize)+"_order_"+str(order)+"_iteration:"+str(0)
    ...
    for label in labelWords:
        if label in labeledData.getBaseLabels():
            print "{}:{}".format(label,labelWords[label])
    ...
    #Testing
    filename_test = "data/generalSentences.csv"
    testData = readLabeledData(filename_test).getLabledTuples()
    testSentences = testData.keys()
    
    testProcessedSentences = (labeledSentencesProcessor(testSentences,order,windowSize)).getSentences()
    
    
    testResultStrings = model.test(testProcessedSentences,outputfile)
    
    for x in range(len(testSentences)):
        testTuple = testData[testSentences[x]]
        s = testProcessedSentences[x]
        os = testSentences[x]
        ...
        print trueBeginTime
        print predictedBeginTime
        print trueEndTime
        print predictedEndTime
        
        beginTimestring = str(beginTime['month'])+"/"+str(beginTime['day'])+"/"+str(beginTime['year'])+" "+str(beginTime['hour'])+":"+str(beginTime['minute'])+":"+str(beginTime['second'])+"\n"    
        endTimestring = str(endTime['month'])+"/"+str(endTime['day'])+"/"+str(endTime['year'])+" "+str(endTime['hour'])+":"+str(endTime['minute'])+":"+str(endTime['second'])+"\n"
...
def loadTranslateTestLabeledData(printResultsToFile, outputFile):
    #HMMOrder
    order = 1
    ...
    #Results Output
    outputfile = "results/output_"+str(windowSize)+"_order_"+str(order)+"_iteration:"+str(0)
    testOutputFile = "results/outputPredictedTimeRange_"+str(windowSize)+"_order_"+str(order)+"_iteration:"+str(0)    
    outputMismatchBeginFile = "results/BeginTimeMismatchSentences_"+str(windowSize)+"_order_"+str(order)+"_iteration:"+str(0)
    outputMismatchEndFile = "results/EndtimeMismatchSentences_"+str(windowSize)+"_order_"+str(order)+"_iteration:"+str(0)
    ...
    for x in xrange(len(test)):
        ...
        ft.write(actualSentence)
        ft.write(s)
        ft.write("\n")
        ft.write(t)
        ft.write("\n")
        timeTupleString = timeTuple[0] + "," +timeTuple[1]+","+timeTuple[2]+"\n"
        ft.write(timeTupleString)           
        ...
        predictedResults.append((os,s,t,predictedBeginTime,trueBeginTime,predictedEndTime,trueEndTime))            
        ...
        ft.write(beginTimestring) 
        ft.write(endTimestring)
...
def main():
    loadTranslateTestLabeledData(True,"")
